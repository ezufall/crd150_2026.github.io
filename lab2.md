...
<p class="comment">**Assignment 2 is due by 12:00 pm, Jan 21 on Canvas.**   See [here](https://ezufall.github.io/crd150_2026.github.io/hw_guidelines.html) for assignment guidelines. You must submit an `.Rmd` file and its associated `.html` file. Name the files: yourLastName_firstInitial_asgn02. For example: zufall_e_asgn02.</p>    
...
You can also manually set the working directory by clicking on Session -> Set Working Directory -> Choose Directory on a Mac or Tools -> Change Working Directory on a Windows machine. Be careful to consider the side effects of changing your working directory, specifically relative file references in your code (for datasets, source files, etc) will become invalid when you change working directories. Because these side effects can cause confusion and errors, it's usually best to start within the working directory associated with your project and remain there for the duration of your session.
...
## $ name  <chr> "Alameda County", "Alpine County", "Amador County", "Butte Count…
## $ state <chr> "California", "California", "California", "California", "Califor…
## $ hisp  <dbl> 367041, 117, 4943, 35445, 5166, 12539, 284003, 5281, 23279, 5094…
## $ asian <dbl> 468356, 8, 565, 9852, 527, 328, 177544, 741, 7805, 96128, 729, 3…
## $ black <dbl> 175063, 18, 764, 3282, 258, 186, 93140, 484, 1614, 45217, 212, 1…
## $ white <dbl> 524881, 777, 29571, 164184, 36932, 7800, 504818, 17233, 145153, …
## $ tpop  <dbl> 1629615, 1203, 37306, 225207, 45057, 21479, 1123678, 27442, 1850…
```
...
```
## # A tibble: 58 × 8
##    GEOID name                state        hisp  asian  black  white    tpop
...
```
## [1] "GEOID" "name"  "state" "hisp"  "asian" "black" "white" "tpop"
```

Let's keep *GEOID*, *name*, *hisp*, *asian*, *black*, *white*, and *tpop*


``` r
select(counties, GEOID, name, hisp, asian, black, white, tpop)
```

```
## # A tibble: 58 × 7
##    GEOID name                  hisp  asian  black  white    tpop
...
``` r
select(counties, GEOID, name, hisp:tpop)
```
...
The `:` operator tells R to select all the variables from *hisp* to *tpop*.  This operator is useful when you've got a lot of variables to keep and they all happen to be ordered sequentially.
...
``` r
mutate(counties1, pwhite = white/tpop, pasian = asian/tpop, 
              pblack = black/tpop, phisp = hisp/tpop)
```
...
``` r
counties1 <- mutate(counties1, pwhite = white/tpop, pasian = asian/tpop, 
              pblack = black/tpop, phisp = hisp/tpop,
              diff = pwhite-phisp)
```
...
In addition to comparison operators, filtering may also utilize logical operators that make multiple selections.  There are three basic logical operators: `&` (and), `|` (or), and `!` (not).  We can keep counties with *phisp* greater than 0.5 **and** *pasian* greater than 0.05 by using `&`.


``` r
filter(counties1, phisp > 0.50 & pasian > 0.05)
```
...
```
## # A tibble: 58 × 12
##    GEOID name       hisp asian black  white   tpop pwhite  pasian  pblack  phisp
##    <chr> <chr>     <dbl> <dbl> <dbl>  <dbl>  <dbl>  <dbl>   <dbl>   <dbl>  <dbl>
##  1 06105 Trinity …   937   141   104  10796  13037  0.828 0.0108  0.00798 0.0719
##  2 06063 Plumas C…  1599   148   160  15640  18724  0.835 0.00790 0.00855 0.0854
##  3 06057 Nevada C…  9109  1090   541  84389  98838  0.854 0.0110  0.00547 0.0922
##  4 06089 Shasta C… 17218  5195  1951 143919 178919  0.804 0.0290  0.0109  0.0962
##  5 06003 Alpine C…   117     8    18    777   1203  0.646 0.00665 0.0150  0.0973
##  6 06091 Sierra C…   290     0     4   2509   2885  0.870 0       0.00139 0.101 
##  7 06043 Mariposa…  1870   168   215  14309  17658  0.810 0.00951 0.0122  0.106 
##  8 06023 Humboldt… 14986  3886  1389 101460 135490  0.749 0.0287  0.0103  0.111 
##  9 06009 Calavera…  5166   527   258  36932  45057  0.820 0.0117  0.00573 0.115 
## 10 06109 Tuolumne…  6385   593   924  43565  53899  0.808 0.0110  0.0171  0.118 
## # ℹ 48 more rows
...
##    GEOID name       hisp  asian  black  white   tpop pwhite pasian  pblack phisp
##    <chr> <chr>     <dbl>  <dbl>  <dbl>  <dbl>  <dbl>  <dbl>  <dbl>   <dbl> <dbl>
##  1 06025 Imperia… 1.50e5   2263   4109  20372 1.80e5  0.113 0.0126 0.0228  0.834
##  2 06107 Tulare … 2.92e5  14622   5973 135372 4.59e5  0.295 0.0319 0.0130  0.636
##  3 06069 San Ben… 3.46e4   1564    437  20872 5.87e4  0.356 0.0267 0.00745 0.589
##  4 06011 Colusa … 1.25e4    328    186   7800 2.15e4  0.363 0.0153 0.00866 0.584
##  5 06047 Merced … 1.56e5  19593   8007  77092 2.67e5  0.288 0.0733 0.0299  0.582
##  6 06053 Montere… 2.51e5  24213  10621 132600 4.33e5  0.306 0.0559 0.0245  0.579
##  7 06039 Madera … 8.79e4   3035   4753  54145 1.54e5  0.351 0.0197 0.0308  0.569
##  8 06031 Kings C… 8.07e4   5463   8916  49728 1.50e5  0.331 0.0364 0.0594  0.537
##  9 06019 Fresno … 5.09e5  96128  45217 293677 9.72e5  0.302 0.0989 0.0465  0.524
## 10 06071 San Ber… 1.11e6 142802 168985 632557 2.12e6  0.298 0.0673 0.0797  0.523
## # ℹ 48 more rows
## # ℹ 1 more variable: diff <dbl>
```
...
counties2 <- counties %>% 
            select(-state) %>% 
            mutate(pwhite = white/tpop, pasian = asian/tpop, 
                    pblack = black/tpop, phisp = hisp/tpop,
                    diff = pwhite-phisp) %>% 
