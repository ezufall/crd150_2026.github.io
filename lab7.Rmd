...
See ACS codebook for the full list: https://api.census.gov/data/2019/acs/acs5/variables.html
...
ca.tracts <- get_acs(geography = "tract", 
              ...
              variables = c(tpop = "B03002_001", 
                            ...
                            ai_an = "B03002_005", nhopi = "B03002_007",
...
ca.tracts <- ca.tracts %>% 
  mutate(pwhite = 100*(whiteE/tpopE), 
        ...
        pai_an = 100*(ai_anE/tpopE),
        pnhopi = 100*(nhopiE/tpopE),
        pasian = 100*(asianE/tpopE), 
        phisp = 100*(hispE/tpopE)
        
        ) %>%
  rename(white = whiteE, black = blackE,
         ai_an = ai_anE, nhopi = nhopiE, asian = asianE, hisp = hispE,  tpop = tpopE) %>%
  select(GEOID,tpop, pwhite, pblack, pai_an, pnhopi, pasian, phisp, 
           white, black, ai_an, nhopi, asian, hisp)  
...
Before calculating segregation, you should map neighborhood racial/ethnic composition in order to gain a visual understanding of how race/ethnic groups are spatially distributed in your study region. For example, let's map percent Hispanic population in Fresno.
...
large.tracts %>%
  ...
  tm_shape(unit = "mi") +
   ...
   tm_title("Percent Hispanic Population in Fresno City Tracts, 2019-2023") +
   tm_layout(scale = 0.6, frame = FALSE) 
```

How does this spatial distribution compare to percent non-Hispanic white population?
...
large.tracts %>%
  ...
  tm_shape(unit = "mi") +
    ...
    tm_scalebar(breaks = c(0, 1, 2), 
                ...
                position = tm_pos_in("right", "bottom")) + 
   tm_compass(type = "4star", position = tm_pos_in("right", "top")) +
   tm_title("Percent White Population in Fresno City Tracts, 2019-2023") +
   tm_layout(scale = 0.6, frame = FALSE)
```
...
It looks like a North/South divide.  Choose another racial/ethnic group and one of the other three cities to map. 
