...
d. Instead of examining segregation at the city level, let’s find where it exists at the neighborhood level. Calculate the Location Quotient for Racial Residential Segregation (LQRSS) for the Asian population for each city. (1 point)
e. Show presentation-ready maps of the Asian LQRSS for each city. (2 points)
...
2. We created a gentrification measure for the City of Fresno in Lab 8b. This question asks you to use the same method from Lab (Ding et al., 2016) to create a gentrification measure for the City of Sacramento, which has received a lot of [local media attention](https://www.abc10.com/article/news/local/sacramento/sacramento-gentrification-neighborhood/103-76b28e16-4622-4733-a04c-e31f5cff7d0b) for the rising presence of gentrification in certain neighborhoods. Download the file *assignment8q2.zip* from Canvas (Files - Week 8 - Assignment) into the appropriate folder on your hard drive. Unzip the file and read in the file *saccitygent.shp*, which is a shapefile of Sacramento census tract boundaries and demographic and socioeconomic variables. Consider the dataset to be cleaned and ready for analysis. A record layout of the data can be found [here](https://raw.githubusercontent.com/crd150/data/master/sac_city_tracts_q2_record_layout.txt).

a. Create a variable that determines whether a neighborhood is not eligible for gentrification, is eligible and is not gentrifying, and is eligible and is gentrifying using the Ding et al., measure. (3 points)

b. Show a presentation-ready table of the percentages of tracts in Sacramento that were not eligible to gentrify, experienced gentrification and did not experience gentrification between 2010 and 2019. (2 points)

c. Produce a presentation-ready map of the gentrification variable you created in (a). From the map, summarize the spatial patterns of gentrification in the city. (2 points)
