...
We've got permanent holes in Sacramento because we've physically removed census tracts with missing values.  Of course, we can shade the missing tracts a certain color and label this in the legend.  We'll do that in [Lab 4](https://ezufall.github.io/crd150_2026.github.io/lab4.html).  But, let's see how imputing the mean works.

To impute the mean, we need to use the tidy friendly `impute_mean()` function in the **naniar** package. Install this package if you haven't already done so. 
...
sac.city.tracts.sf.mn <- sac.city.tracts.sf %>%
    mutate(psubhous = impute_mean(psubhous))
```
